...
```python
MARKET_INDEX = "QQQM"                                    # 시장을 볼 기준. 매매하지 않습니다
DIP_BUY = {"TQQQ": {"entry_dip": -10.0, "index_sma_days": 200}}
TAKE_PROFIT_PCTS = {"TQQQ": None}                        # %로 익절하지 않습니다
```

- **진입** — 기준 종목이 52주 고점 대비 `entry_dip` 이하로 빠졌고, **그러면서도 200일선 위**일 때
- **익절** — 그 종목이 **자기 52주 고점을 회복**하면. %가 아니라 값입니다
- **손절** — `STOP_LOSS_PCTS`의 값

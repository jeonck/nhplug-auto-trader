...
4. **예약 만들기** → 클로드 코드에게 "예약 만들어 줘" 라고 말하면 1시간 간격으로
   대신 만들어 줍니다 (예약 기능이 없으면 cron)

컴퓨터를 켜 두기 어렵다면 → [컴퓨터를 꺼 두고 싶다면](#컴퓨터를-꺼-두고-싶다면-원격-서버)

## 예약이 하는 일

터미널에 프로그램을 띄워 두지 않습니다. **클로드 코드 예약이 1시간마다** 부릅니다.
...
| 지금 살까 팔까 | **클로드 코드** — 1시간마다 예약이 판단 | 클로드 코드 예약 |
...
```mermaid
flowchart TD
    A["클로드 코드 예약이 1시간마다<br/>python trade.py --scan"] --> B{"장이 열렸나"}

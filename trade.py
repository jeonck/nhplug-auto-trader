...
CANDLES = 20  # 클로드 코드에게 보여 줄 종가 개수. 흐름을 보기에 이 정도면 충분합니다.


def needs_judgment(m):
    """클로드 코드의 판단이 필요한 종목인가. 규칙만으로 결론이 나면 묻지 않습니다.
...
def scan(act, dry=False):
    ...
    클로드 코드 예약이 회차마다 이것부터 부릅니다. 손절·익절과 사지 않을 종목은 여기서
    끝나고, **판단이 필요한 종목만** 남겨서 돌려줍니다. 클로드 코드가 늘 생각할 필요가
...
def facts(m):
    """클로드 코드에게 보여 줄 한 종목의 사실. 지표로 요약하지 않고 숫자를 그대로 줍니다."""
    out = {
        "종목코드": m["code"],
        "이름": m["name"],
...
def do(act, calls):
    ...
    for code, call in calls.items():
        ...
        no_reason = "클로드 코드가 이유를 적지 않았습니다"
        m["ai"] = {"decision": decision, "reason": reason or no_reason}
...
USAGE = """이 프로그램은 클로드 코드 예약이 1시간마다 부릅니다.

  python trade.py --scan   지금 상황을 살펴 판단이 필요한 종목을 알려 줍니다
  python trade.py --do ..  클로드 코드가 정한 판단대로 주문합니다

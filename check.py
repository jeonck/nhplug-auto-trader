...
클로드 코드가 전략을 고친 뒤 이걸 돌려서 확인합니다. 여기서 막으면 장중에 안 터집니다.
...
# 사고팔지는 클로드 코드가 정합니다. 그 답이 왔을 때와 못 받았을 때를 모두 넣어 봅니다.
_BUY = {"decision": "buy",
...
CASES = [
    ...
    # 클로드 코드가 뭐라 하든 손절·익절 선은 규칙이 지킵니다.
    # 여기서는 hold라고 답해도 팔아야 합니다.
    ...
    ("국내 고점부근", {**_KR, "held": False, "price": 374000, "qty": 0, "avg": 0,
                  "pnl_pct": 0.0, "ai": _BUY}),
    # 클로드 코드가 없거나 답이 이상한 날. 모르는 상태를 "괜찮다"로 읽으면 안 됩니다.
